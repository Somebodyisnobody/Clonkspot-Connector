...
public class RemoveIgnoredHostCommand implements ServerCommand {
    ...
    public void performCommand(PrivateChannel channel, String[] args) {
        ...
        if (Controller.INSTANCE.configuration.removeIgnoredHostname(hostname)) {
            channel.sendMessage(":white_check_mark: Host deleted.").queue();
            Controller.INSTANCE.log.addLogEntry("DiscordConnector: Ignored host \"" + hostname + "\" removed by \"" + channel.getUser().getName() + "\".");
        } else {
            channel.sendMessage(":x: Error: Hostname not found.").queue();
        }
    }
}
